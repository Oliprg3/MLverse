...
export function Modal({ open, onClose, title, subtitle, children, footer, className, size = "lg" }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div
        className={cn(
          "relative flex max-h-[88vh] w-full flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl animate-modal-in",
          maxW,
          className,
        )}
      >
        {(title || subtitle) && (
          <div className="flex items-start justify-between gap-4 border-b border-border px-6 py-4">
            ...
            <button
              onClick={onClose}
              className="-mr-1 shrink-0 rounded-xl p-1.5 text-muted transition-colors hover:bg-foreground/[0.05] hover:text-foreground"
              aria-label="Close"
            >

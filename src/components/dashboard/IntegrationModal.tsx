...
type AgentPhaseConfig = { id: Exclude<AgentPhase, "idle">; label: string; status: string; icon: typeof BrainCircuit };

const agentPhases: AgentPhaseConfig[] = [
  { id: "analyzing", label: "Analyzing", status: "Reading your request and model context", icon: BrainCircuit },
  { id: "planning", label: "Planning", status: "Mapping the experience and field structure", icon: WandSparkles },
  { id: "generating", label: "Generating", status: "Building a responsive prediction surface", icon: Blocks },
  { id: "validating", label: "Validating", status: "Checking fields, copy, and interactions", icon: CheckCircle2 },
  { id: "completing", label: "Completing", status: "Applying the final preview changes", icon: CheckCircle2 },
];
      ...
      <div className="grid h-[76vh] min-h-[560px] max-h-[820px] min-w-0 grid-rows-[minmax(0,1fr)_minmax(0,1fr)] overflow-hidden lg:grid-cols-[minmax(380px,1.22fr)_minmax(285px,0.78fr)] lg:grid-rows-1">
        <section className="order-2 flex min-h-0 min-w-0 flex-col border-t border-border lg:order-2 lg:border-l lg:border-t-0 lg:border-r-0">
          ...
          <div ref={transcriptRef} aria-live="polite" className="scroll-thin min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
            {messages.length === 0 ? <div className="rounded-xl border border-dashed border-border bg-foreground/[0.02] p-5 text-center"><WandSparkles className="mx-auto h-6 w-6 text-sky-400" /><p className="mt-3 text-xs font-medium text-foreground-2">Start with a design direction</p><p className="mt-1 text-[11px] leading-relaxed text-muted">Load a model, then ask the builder to shape the experience around your users.</p></div> : null}
            {messages.length > 0 ? <div className="space-y-3">{messages.map((message, index) => <div key={`${message.role}-${index}`} style={{ animationDelay: `${Math.min(index, 6) * 45}ms` }} className={`animate-message-in flex gap-2 ${message.role === "user" ? "flex-row-reverse" : ""}`}><span className="mt-0.5 shrink-0 text-muted-2">{message.role === "user" ? <UserRound aria-hidden className="h-4 w-4" /> : <Bot aria-hidden className="h-4 w-4 text-sky-400" />}</span><div className={`min-w-0 max-w-[88%] rounded-lg px-3 py-2 transition-colors ${message.role === "user" ? "bg-sky-400/10 text-foreground" : "bg-foreground/[0.04] text-foreground-2"}`}><p className="break-words text-xs leading-relaxed">{message.content}</p><time className="mt-1 block text-[9px] text-muted" dateTime={message.time}>{message.time}</time>{requestError && index === messages.length - 1 ? <button type="button" onClick={() => lastRequestRef.current && void send(lastRequestRef.current)} className="mt-2 inline-flex items-center gap-1 text-[10px] font-medium text-sky-300 transition-colors hover:text-sky-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><RefreshCw className="h-3 w-3" /> Retry request</button> : null}</div></div>)}</div> : null}
            {activities.length > 0 ? <div className="animate-message-in rounded-lg border border-border bg-muted/20 p-2.5"><div className="flex items-center justify-between gap-2 px-1"><span className="flex items-center gap-1.5 text-[10px] font-semibold text-foreground-2"><BrainCircuit className="h-3.5 w-3.5 text-muted" /> Thinking</span><span className="text-[9px] text-muted">{loading ? "Live workflow" : "Finished"}</span></div><div ref={thinkingRef} className="scroll-thin mt-1 max-h-40 space-y-1 overflow-y-auto pr-1">{activities.map((activity, index) => <details key={`${activity.kind}-${index}`} open={activity.status === "running"} className="group rounded-md border border-transparent transition-colors open:border-border/70 open:bg-background/30"><summary className="flex cursor-pointer list-none items-center gap-2 rounded-md px-2 py-1.5 text-[10px] text-foreground-2 transition-colors hover:bg-foreground/[0.04] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><span className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-full ${activity.status === "running" ? "bg-sky-400/10 text-sky-300" : "bg-emerald-400/10 text-emerald-300"}`}>{activity.kind === "thought" ? <BrainCircuit className="h-3 w-3" /> : activity.kind === "tool" ? <Blocks className="h-3 w-3" /> : <CheckCircle2 className="h-3 w-3" />}</span><span className="min-w-0 flex-1 truncate">{activity.title}</span>{activity.status === "running" ? <Loader2 className="h-3 w-3 shrink-0 animate-spin text-sky-400" /> : <Check className="h-3 w-3 shrink-0 text-emerald-400" />}</summary><p className="animate-fade-in break-words px-8 pb-2 text-[10px] leading-relaxed text-muted">{activity.detail}</p></details>)}</div></div> : null}
            {loading ? <div className="animate-message-in flex gap-2"><Bot className="mt-0.5 h-4 w-4 shrink-0 text-sky-400" /><div className="rounded-lg bg-foreground/[0.04] px-3 py-2.5"><div className="flex items-center gap-1"><span className="h-1.5 w-1.5 animate-dot-bounce rounded-full bg-sky-400" /><span className="h-1.5 w-1.5 animate-dot-bounce rounded-full bg-sky-400 [animation-delay:120ms]" /><span className="h-1.5 w-1.5 animate-dot-bounce rounded-full bg-sky-400 [animation-delay:240ms]" /><span className="ml-1 text-[10px] text-muted">{activePhase?.status ?? "Agent is working"}</span></div></div></div> : null}
          </div>

      ...
      <div className="grid h-[76vh] min-h-[560px] max-h-[820px] min-w-0 grid-rows-[minmax(0,1fr)_minmax(0,1fr)] overflow-hidden lg:grid-cols-[minmax(380px,1.22fr)_minmax(285px,0.78fr)] lg:grid-rows-1">
        <section className="order-2 flex min-h-0 min-w-0 flex-col border-t border-border lg:order-2 lg:border-l lg:border-t-0 lg:border-r-0">
          ...
          <div ref={transcriptRef} aria-live="polite" className="scroll-thin min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
            ...
            {loading ? <div className="animate-message-in flex items-center gap-2 text-[11px] text-muted"><CircleNotch size={13} className="shrink-0 animate-spin text-sky-400" />{activePhase?.status ?? "Agent is working"}</div> : null}
          </div>
          {model ? <div className="flex flex-wrap gap-x-3 gap-y-1 border-t border-border px-4 py-2.5">{suggestions.map((suggestion) => <button key={suggestion} type="button" onClick={() => applySuggestion(suggestion)} disabled={loading} className="max-w-full truncate text-[11px] text-muted transition-colors hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50">{suggestion}</button>)}</div> : null}

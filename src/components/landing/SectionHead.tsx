    ...
    <div className={`mb-16 ${centered ? "mx-auto max-w-3xl text-center" : "max-w-3xl"}`}>
      <Reveal>
        <div className={`mb-4 flex items-center gap-3 ${centered ? "justify-center" : ""}`}>
          ...
          <span className="font-mono text-[11px] uppercase tracking-[0.28em] text-zinc-500">{kicker}</span>
        </div>
      </Reveal>
      <Reveal delay={100}>
        <h2 className="text-balance text-3xl font-semibold tracking-[-0.02em] text-white sm:text-4xl md:text-5xl">
          {title}
        </h2>
      </Reveal>
      {copy ? (
        <Reveal delay={200}>
          <p className={`mt-5 text-pretty leading-relaxed text-zinc-400 ${centered ? "mx-auto max-w-2xl" : ""}`}>
            {copy}
          </p>
        </Reveal>
      ) : null}

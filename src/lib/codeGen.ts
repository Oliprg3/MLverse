...
function generateSklearn(g: GraphPayload): string {
  ...
  const header = [
    '"""',
    "Auto-generated by Datlify AI Canvas.",
...
model = Model().to(DEVICE)`,
  ...
  const header = [
    '"""',
    "Auto-generated by Datlify AI Canvas (PyTorch training script).",
...
export function generateCode(g: GraphPayload): GeneratedCode {
  ...
  return {
    ...
    filename: colab ? "datlify_pytorch_train.py" : "datlify_pipeline.py",
    language: "python",
  };
}
...
export function generateNodeServer(g: GraphPayload): GeneratedCode {
  const filename = "datlify_service.mjs";
  const header = [
    "/**",
    " * Auto-generated by Datlify AI Canvas — inference microservice.",
    ` * Pipeline: ${g.nodes.map((n) => n.label).join(" -> ") || "(empty)"}`,
    " * Setup:    npm install express   ·   python -m pip install scikit-learn pandas joblib",
    " * Run:      node datlify_service.mjs",
    " */",
  ].join("\n");

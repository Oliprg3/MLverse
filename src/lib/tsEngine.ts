...
/* ── Live preview model for the AI Builder ────────────────────────────────── */
...
export interface PreviewModel {
  ...
  classNames: string[];
  trainedOn: number;
  predict(features: number[]): PreviewPrediction | null;
}

/**
 * Train a lightweight in-browser model (Gaussian NB / KNN, same as the
 * instant engine) on the saved pipeline's dataset so the AI Builder preview
 * can serve REAL predictions from the form inputs — no server required.
 */
export function trainPreviewModel(graph: GraphPayload): PreviewModel | null {
  try {
    const nodes = graph.nodes as unknown as {
      type: string;
      category: string;
    ...
    const enc = buildDataset(nodes);
    const nClasses = enc.classNames.length;
    if (enc.X.length < 4 || nClasses < 2) return null;

    const hasScaler = nodes.some((n) => n.type === "pre:scaler");
    const scaler = hasScaler ? standardizeFit(enc.X) : null;
    const Xs = scaler ? enc.X.map(scaler.transform) : enc.X;

    const modelNode = nodes.find((n) => n.category === "classic_ml");
    const isKnn = modelNode?.type === "ml:knn";
    const k = Math.max(1, Math.round(Number((modelNode?.params as Record<string, number> | undefined)?.n_neighbors ?? 5)));
    const gnb = isKnn ? null : trainGnb(Xs, enc.y, nClasses);

    const medianImpute = (values: number[]): number[] => {
      // Replace non-finite entries with the feature median so typos don't crash inference.
      for (let j = 0; j < values.length; j++) {
        if (!Number.isFinite(values[j])) {
          const col = Xs.map((r) => r[j]).filter(Number.isFinite).sort((a, b) => a - b);
          values[j] = col.length ? col[Math.floor(col.length / 2)] : 0;
        }
      }
      return values;
    };
    ...
    return {
      ...
      classNames: enc.classNames,
      trainedOn: enc.X.length,
      predict(features: number[]): PreviewPrediction | null {
        if (features.length !== enc.featureNames.length) return null;
        const row = medianImpute(features.slice());
        const scaled = scaler ? scaler.transform(row) : row;
        const proba = isKnn
          ? knnPredictProba(Xs, enc.y, [scaled], k, nClasses)[0]
          : predictProba(gnb!, [scaled])[0];
        let best = 0;
        proba.forEach((p, i) => { if (p > proba[best]) best = i; });
        return {
          label: enc.classNames[best],
          confidence: proba[best],
          probabilities: proba
            .map((p, i) => ({ label: enc.classNames[i], p }))
            .sort((a, b) => b.p - a.p),
        };
      },
    };
